...
const PageSelector = (props) => {
  ...
  return Math.min(props.total - 1, Math.max(0, page)) !== page && setPage((page2) => Math.min(props.total - 1, Math.max(0, page2))), /* @__PURE__ */ jsxRuntime.jsxs(jsxRuntime.Fragment, { children: [
    /* @__PURE__ */ jsxRuntime.jsx(ui.Button, { icon: icons.ChevronLeftIcon, mode: "bleed", padding: 3, style: { cursor: "pointer" }, disabled: page <= 0, onClick: () => {
    ...
    /* @__PURE__ */ jsxRuntime.jsx(ui.Button, { icon: icons.ChevronRightIcon, mode: "bleed", padding: 3, style: { cursor: "pointer" }, disabled: page >= props.total - 1, onClick: () => {
      setPage((page2) => Math.min(props.total - 1, Math.max(0, page2 + 1)));
    } })
  ] });
};
function VideosBrowser({ onSelect }) {
  const { assets, isLoading, searchQuery, setSearchQuery, setSort, sort } = useAssets(), [page, setPage] = React.useState(0), pageLimit = 18, pageTotal = Math.floor(assets.length / pageLimit) + 1, [editedAsset, setEditedAsset] = React.useState(null), freshEditedAsset = React.useMemo(
...
function InputBrowser({
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(
    StyledDialog,
    {
      ...
      onClose: handleClose,
      width: 2,
      children: /* @__PURE__ */ jsxRuntime.jsx(SelectAssets, { asset, onChange, setDialogState, config })
    }
  );
}
